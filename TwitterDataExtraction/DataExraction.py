...
provinces = [Saskatchewan, Alberta, BritishColumbia, Nunavut, NorthwestTerritories, Yukon, Ontario, Québec, NewBrunswick, NovaScotia, NewfoundlandAndLabrador, Manitoba, PrinceEdwardIsland]
...
for province in provinces:
    ...
    tweets = tweepy.Cursor(api.search, q = "NBA", count = 100, geocode = province.geoCode,
                           lang = "en",
                           since = "2019-01-01").items()
    # for tweet in tweets:
    #     print (tweet.created_at, tweet.text)

    tweetsByProvince.append({province.name, tweets})
